...
最近有定期從遠端 Linux 主機上拉下大量檔案的需求，內容是遊戲開發的資源檔，每次數百GB。因為檔案實在太大，不得不找一些方法壓縮檔案，以期縮短下載時間。
...
挑選的標準很簡單，不要太冷門，壓縮/解壓縮工具在 Windows/Linux 雙平台上可以方便操作。如果有人知道什麼特別優秀的壓縮演算法麻煩告訴我。
...
從表格來看，7zip (也就是LZMA2演算法) 的壓縮比第一，GZIP/ZIP 次之，LZ4 的壓縮率最差。壓縮速度則是 LZ4 最快，7zip 次之，Zip/Gzip 最慢。

除了壓縮時間以外，還要考慮下載時間。所以根據我家的平均網速 4.5MB/sec，計算出下載檔案需要的時間，兩者加總起來：
...
結果出來了。綜合來看，**7zip 最快速壓縮**是最佳方案，只要167分鐘就可以把整包129GB檔案下載到我的本機。

## 資源消耗方面的比較

CPU 使用量：依我的粗略觀察，LZ4 CPU 使用量最低；Gzip/Zip 會吃滿一個單核心；7zip 則是充分利用全部核心，八個核心全部 100%。所以雖然 7zip 總時間最短，但是代價是吃光全部主機資源。

7zip從最快速壓縮 `-mx1` 換成一般壓縮 `-mx3` 只增加了 1% 壓縮率，但是壓縮時間長了一倍，不值得。

LZ4 演算法標榜超快速壓縮，名符其實，只需八分多鐘壓完全部檔案，海放其他演算法。但是因為壓縮率普通，所以需要多花時間下載。如果下次壓比較小的檔案，我可能會考慮LZ4。
